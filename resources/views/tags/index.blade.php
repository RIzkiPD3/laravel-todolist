                ...
                <div class="p-6 text-gray-900">
                    ...
                    @if ($tags->isEmpty())
                        <div class="text-center py-8">
                            <p class="text-gray-500 mb-4">{{ __('You have no tags yet.') }}</p>
                            ...
                            <table class="min-w-full divide-y divide-gray-200">
                                <thead class="bg-gray-50">
                                    <tr>
                                        ...
                                        <th scope="col" class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">{{ __('Tasks') }}</th>
                                ...
                                <tbody class="bg-white divide-y divide-gray-200">
                                    @foreach ($tags as $tag)
                                        <tr>
                                            ...
                                            <td class="px-6 py-4 whitespace-nowrap">
                                                <div class="text-sm text-gray-500">{{ $tag->tasks->count() }}</div>
                                            </td>
                                            <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                                <a href="{{ route('tags.show', $tag) }}" class="text-indigo-600 hover:text-indigo-900 mr-3">{{ __('View') }}</a>
                                                @if (!$tag->is_default)
                                                    <a href="{{ route('tags.edit', $tag) }}" class="text-yellow-600 hover:text-yellow-900 mr-3">{{ __('Edit') }}</a>
                                                    <form action="{{ route('tags.destroy', $tag) }}" method="POST" class="inline">
                                                        @csrf
                                                        @method('DELETE')
                                                        <button type="submit" class="text-red-600 hover:text-red-900" onclick="return confirm('{{ __('Are you sure you want to delete this tag?') }}')">{{ __('Delete') }}</button>
                                                    </form>
                                                @endif
                                            </td>
                                        </tr>
